...
LogoutCtrl.$inject = ['$scope', '$http', '$window', '$sce'];

function LogoutCtrl($scope, $http, $window, $sce) {
    $scope.logout = function (email) {
        $http({
            ...
            function successCallback(response) {
                $window.location.href = 'https://localhost:8443' + response.data; //https://localhost:8443/login
