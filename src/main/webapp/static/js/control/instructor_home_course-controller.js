...
}).controller('InstructorHomeCourseController', InstructorHomeCourseCtrl);
InstructorHomeCourseCtrl.$inject = ['$scope', '$http', '$location', '$sce'];

function InstructorHomeCourseCtrl($scope, $http, $location, $sce) {
    let emailUri = $location.absUrl().toString().split('/')[5]; //https://localhost:8443/student/student_home/{email}
    $http({
        method: 'GET',
        port: 8443,
        url: 'https://localhost:8443/instructor/' + emailUri + "/my_course_info", //FIXME
